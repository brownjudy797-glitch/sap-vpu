...
def validate(
    ...
    return inputs, weights, expected, bias, multiplier, shift, lut, gelu
...
def self_test() -> None:
    ...
    inputs, weights, expected, bias, multiplier, shift, lut, gelu = validate(raw)
    assert expected == [
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        write_include(output, inputs, weights, expected, bias, multiplier, shift, lut, gelu)
...
def main() -> int:
    ...
    try:
        if args.self_test:
            ...
            inputs, weights, expected, bias, multiplier, shift, lut, gelu = validate(
                json.loads(args.fixture.read_text(encoding="ascii"))
            )
            write_include(args.asm, inputs, weights, expected, bias, multiplier, shift, lut, gelu)
