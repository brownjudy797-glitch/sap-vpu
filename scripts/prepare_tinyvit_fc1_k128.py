...
def fc2_structured_window_outputs(
    ...
    masks = fc2_structured_group_masks(weights)
    return [
        [
            [
                sum(
                    ...
                    if masks[k // CHUNK_K] & (1 << (output * (CHUNK_K // 4) + (k % CHUNK_K) // 4))
                )
                for output in range(FC2_OUTPUT_CHANNELS)
            ]
            for token in range(TOKENS)
...
def write_include(
    ...
    sparse_masks = fc2_structured_group_masks(fc2_weights)
    sparse_mac_active = sum(
        bin(mask).count("1") * TOKENS for mask in sparse_masks[: WINDOW_OUTPUT_CHANNELS // CHUNK_K]
    )
    lines = [
        ...
        f".equ TINYVIT_FC2_SPARSE_MAC_ACTIVE, {sparse_mac_active}",
        f".equ TINYVIT_FC2_SPARSE_DMA_READ_SAVED, {WINDOW_OUTPUT_CHANNELS // CHUNK_K}",
    ]
    ...
    lines.extend((".balign 4", "tinyvit_fc2_sparse_descriptors:"))
    lines.extend(
        f"  .word 0x{VTDMA_METADATA_ENABLE | (mask << 4) | 0x9:x}" for mask in sparse_masks
    )
    lines.extend((".balign 4", "tinyvit_fc2_sparse_expected:"))
    for window in fc2_structured_window_outputs(gelu, fc2_weights):
        for row in window:
            lines.extend(f"  .word {value}" for value in row)
...
def self_test() -> None:
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        assert ".equ TINYVIT_FC2_SPARSE_MAC_ACTIVE, 12" in generated
        assert ".equ TINYVIT_FC2_SPARSE_DMA_READ_SAVED, 2" in generated
        assert "tinyvit_fc2_k8_expected:" in generated
        assert "tinyvit_fc2_sparse_descriptors:" in generated
        assert "tinyvit_fc2_sparse_expected:" in generated
