...
def write_board_asm_include(
    path: Path,
    raw: dict[str, Any],
    inputs: list[list[int]],
    pairs: dict[str, Any],
) -> None:
    pair_index = pairs["output_pairs"].index([0, 1])
    weights = pairs["weights"][pair_index]
    lhs_words = chunk_words(inputs, INPUT_CHANNELS)
    rhs_words = chunk_words(weights, INPUT_CHANNELS)
    policies = {}
    for label, policy_name in (
        ("GLOBAL", "layer_global_l1_12p5"),
        ("BUDGET", "layer_l1_budget_5pct"),
    ):
        policy = pairs["policies"][policy_name]
        ...
        policies[label] = {
            "expected": policy["expected"][pair_index],
            "metadata": pack_stream_metadata(masks),
            "mac_active": TOKENS * weight_reads,
            "dma_saved": INPUT_CHANNELS - input_reads - weight_reads,
        }

    provenance = raw.get("provenance", {})
    matrix_bytes = len(lhs_words) * 4
    metadata_bytes = len(policies["GLOBAL"]["metadata"]) * 4
    lines = [
        "/* Generated by prepare_tinyvit_fc2_k512.py; do not edit. */",
        f"/* Model: {provenance.get('model_id', 'unknown')} */",
        f"/* Layer: {provenance.get('layer_id', 'unknown')} */",
        f"/* Checkpoint SHA256: {provenance.get('checkpoint_sha256', 'unknown')} */",
        ".equ TILE_OUTPUT_0, 0",
        ".equ TILE_OUTPUT_1, 1",
        f".equ TILE_K_BLOCKS, {CHUNKS}",
        f".equ TILE_MATRIX_WORDS, {len(lhs_words)}",
        f".equ TILE_METADATA_WORDS, {len(policies['GLOBAL']['metadata'])}",
        ".equ LHS_BASE, 0x00010000",
        f".equ RHS_BASE, 0x{0x10000 + matrix_bytes:08x}",
        f".equ OUT_BASE, 0x{0x10000 + 2 * matrix_bytes:08x}",
        f".equ DESC_BASE, 0x{0x10010 + 2 * matrix_bytes:08x}",
        f".equ GLOBAL_META_BASE, 0x{0x10024 + 2 * matrix_bytes:08x}",
        f".equ BUDGET_META_BASE, 0x{0x10024 + 2 * matrix_bytes + metadata_bytes:08x}",
        f".equ TILE_DENSE_MAC_ACTIVE, {INPUT_CHANNELS}",
        ".equ TILE_DENSE_DMA_SAVED, 0",
    ]
    for row in range(TOKENS):
        for column in range(FC2_OUTPUT_CHANNELS):
            lines.append(
                f".equ TILE_DENSE_EXPECT_{row}{column}, "
                f"{pairs['dense_expected'][pair_index][row][column]}"
            )
...
def self_test() -> None:
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        board_output = Path(directory) / "tile_fixture.inc"
        write_board_asm_include(board_output, raw, inputs, pairs)
        board_generated = board_output.read_text(encoding="ascii")
        assert ".equ TILE_K_BLOCKS, 64" in board_generated
        assert ".equ TILE_GLOBAL_MAC_ACTIVE, 458" in board_generated
        assert ".equ TILE_BUDGET_DMA_SAVED, 32" in board_generated
        assert ".equ BUDGET_META_BASE, 0x00010864" in board_generated
...
def main() -> int:
    ...
    try:
        if args.self_test:
            if args.fixture or args.svh or args.board_inc:
                parser.error("--self-test does not accept fixture output arguments")
            self_test()
        elif not args.fixture or not (args.svh or args.board_inc):
            parser.error("fixture and at least one output are required")
        else:
            raw = json.loads(args.fixture.read_text(encoding="ascii"))
            inputs, weights, expected = validate(raw)
            pairs = validate_pairs(raw, inputs)
            if args.svh:
                write_svh(args.svh, inputs, weights, expected, pairs)
            if args.board_inc:
                write_board_asm_include(args.board_inc, raw, inputs, pairs)
