...
def validate_fixture(raw: Any) -> dict[str, Any]:
    ...
    requant_shift = quantization.get("fc1_output_requant_shift", 0)
    if not isinstance(requant_shift, int) or isinstance(requant_shift, bool) or not 0 <= requant_shift <= 31:
        raise fail("quantization.fc1_output_requant_shift must be an integer from 0 to 31")
...
def requantize_relu(value: int, shift: int) -> int:
    if value < 0:
        return 0
    if shift:
...
def evaluate(
    ...
    total = 0
    requant_shift = fixture["quantization"].get("fc1_output_requant_shift", 0)
    for token_index, token in enumerate(fixture["input_tokens"]):
        hidden_output = [dot(token, weight) for weight in fixture["fc1_weights"]]
        hidden = [requantize_relu(value, requant_shift) for value in hidden_output]
        for hidden_index, value in enumerate(hidden):
            if value > 127:
                raise fail(
                    f"requantized ReLU activation token {token_index}, channel {hidden_index} exceeds signed INT8"
                )
        ...
        "layers": [
            {
                ...
                "post_op": "power-of-two-requantize-relu",
                "requant_shift": fixture["quantization"].get("fc1_output_requant_shift", 0),
...
def write_artifacts(
    ...
    requant_shift = fixture["quantization"].get("fc1_output_requant_shift", 0)
    requant_rounding = 0 if requant_shift == 0 else 1 << (requant_shift - 1)
    ...
    asm_lines = [
        ...
        f".equ TINYVIT_MLP2_FC1_REQUANT_SHIFT, {requant_shift}",
        f".equ TINYVIT_MLP2_FC1_REQUANT_ROUNDING, {requant_rounding}",
    ]
    ...
    write_text(
        ...
        f"localparam int unsigned TINYVIT_MLP2_FC1_REQUANT_SHIFT = {requant_shift};\n"
        f"localparam logic [31:0] TINYVIT_MLP2_FC1_REQUANT_ROUNDING = 32'd{requant_rounding};\n"
...
def self_test() -> int:
    ...
    activation_fixture = validate_fixture(
        json.loads((root / "sw/baremetal/fixtures/tinyvit_mlp2_activation.json").read_text(encoding="utf-8"))
    )
    _, expected, outputs, hidden_outputs, hidden_activations = evaluate(activation_fixture)
    assert expected == 148000
    assert outputs == [[0, 0], [2722, 6528]]
    assert hidden_outputs == [[-21634, -4430, -6361, -11347], [6828, 8762, -1007, -6748]]
    assert hidden_activations == [[0, 0, 0, 0], [53, 68, 0, 0]]
    assert command_mapping(activation_fixture)["layers"][0]["requant_shift"] == 7
