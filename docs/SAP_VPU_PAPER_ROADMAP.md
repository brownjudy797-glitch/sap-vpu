...
| ASIC evidence | Standalone `sap_vpu_core` is mapped with Synopsys DC against the local TSMC28 `tt0p9v85c` `.db` at 10 ns. Policy SAIF runs are reproducible but retain `PWR-452` partial annotation; the current emitted gate netlist also fails the unconnected-output preflight, so absolute power remains preliminary. | `make dc-vpu-policy-power-matrix`, `make dc-vpu-gate-netlist-check`, `docs/SAP_VPU_ASIC_FLOW.md` |

The next milestone is not another interface feature. It is an exported
TinyViT checkpoint or quantized tensor fixture that permits data-accurate
dimensions, scale/error accounting, and a defensible power-reduction claim.
