...
Full output dimensions, bias/GELU execution, and a defensible power-reduction
claim remain subsequent paper-evidence steps. Float references now expose both
the original four-channel MLP2 gap and the K=128 FC1 quantization boundary.
...
2. Execute bias and GELU at an explicit software/hardware boundary for the
   validated K=128 FC1 slice, then widen beyond eight output channels.
