...
| Paper workload | TinyViT policy smokes cover INT8/INT4/INT2, bitmap and software-scheduled sparsity, ablations, counters, reuse, and RAM traffic. The default fixture combines real-image activations and `fc1/fc2` slices from the same timm TinyViT-5M checkpoint. The original 2x4x4x2 MLP2 path remains a partial two-layer smoke. A system smoke executes two tokens across 128 FC1 input/output channels using eight host-loaded windows and CPU bias/Q16/GELU postprocessing. The host policy study covers all 512 inputs and 128 outputs for 6,272 tokens from eight images. Four evenly spaced output pairs now carry those full-layer masks back through exact K=512 subsystem RTL checks, including input and weight read suppression. Runtime window swapping, full-model accuracy, and end-to-end inference remain outside the claim; GELU is not claimed as VPU hardware. | `make tinyvit-fc1-k128-smoke`, `make sim-subsystem-k512-pair-policies`, `make tinyvit-sparsity-study`, `docs/SAP_VPU_MODEL_MAPPING_CONTRACT.md` |
...
2. Run matched gate-SAIF for the representative output-pair policies; keep its
   energy result separate from the full-layer host estimate.
