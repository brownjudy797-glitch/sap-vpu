...
| FPGA evidence | Standalone core and complete `sap_vpu_subsystem` both pass same-mode Artix-7 OOC implementation at 100/140 MHz. At 140 MHz the subsystem uses 2752 LUTs and 1332 FFs with +0.091 ns WNS. A 128-tile post-synth read-compute-write smoke annotates 6452/6461 routed nets and reports 0.011 W dynamic power, but excludes external RAM, CPU, and board power. | `make fpga-vpu-synth`, `make fpga-vpu-subsystem-saif-power`, `docs/SAP_VPU_FPGA_FLOW.md` |
...
2. Broaden the verified FPGA subsystem activity from the repeated M=2, N=2,
   K=4 tile to a larger TinyViT MLP layer slice; keep it separate from board
   and full-SoC power.
