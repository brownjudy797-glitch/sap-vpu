...
| Paper workload | TinyViT policy smokes cover INT8/INT4/INT2, bitmap and software-scheduled sparsity, ablations, counters, reuse, and RAM traffic. The default fixture combines real-image activations and `fc1/fc2` slices from the same timm TinyViT-5M checkpoint. The original 2x4x4x2 MLP2 path remains a partial two-layer smoke. A system smoke executes two tokens across 128 FC1 input/output channels using eight host-loaded windows and CPU bias/Q16/GELU postprocessing. A subsystem path consumes captured GELU activations across the complete 512-channel FC2 input dimension for two tokens and two outputs, checking exact aggregates and transactions. The host-side policy study now covers all 512 inputs and 128 outputs for 6,272 tokens from eight images. Runtime window swapping, full-model accuracy, and end-to-end inference remain outside the claim; GELU is not claimed as VPU hardware. | `make tinyvit-fc1-k128-smoke`, `make sim-subsystem-k512-policies`, `make tinyvit-sparsity-study`, `docs/SAP_VPU_MODEL_MAPPING_CONTRACT.md` |
...
2. Materialize the selected full-layer K=512 masks into representative output-
   pair RTL sweeps, then compare their transaction counts with the host model.
