...
| FPGA evidence | Standalone core and complete `sap_vpu_subsystem` both pass same-mode Artix-7 OOC implementation at 100/140 MHz. At 140 MHz the subsystem uses 2752 LUTs and 1332 FFs with +0.091 ns WNS. A fixture-driven run maps 128 two-layer 2x4x4x2 MLP2 inferences through 384 autonomous tiles and 1536 VDOTs; its post-synth SAIF annotates 6452/6461 routed nets and reports 0.012 W dynamic power and 15.683 nJ/MLP2. ReLU/repacking, external RAM, CPU, and board power are excluded. | `make fpga-vpu-synth`, `make fpga-vpu-subsystem-saif-power`, `docs/SAP_VPU_FPGA_FLOW.md` |
...
2. Replace the synthetic 2x4x4x2 fixture with a checkpoint-derived TinyViT
   projection slice carrying model, layer, quantization, and hash provenance.
3. Extend the mapping to larger dimensions and account for bias, GELU, and
   floating-point error only after the checkpoint-derived slice is reproducible.
