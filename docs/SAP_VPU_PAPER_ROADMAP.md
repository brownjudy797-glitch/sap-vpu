...
| Paper workload | TinyViT policy smokes cover INT8/INT4/INT2, bitmap and software-scheduled sparsity, ablations, counters, reuse, and RAM traffic. The default fixture combines real-image activations and `fc1/fc2` slices from the same timm TinyViT-5M checkpoint. The original 2x4x4x2 MLP2 path remains a partial two-layer smoke. A separate FC1 smoke now executes two tokens across all 128 input channels and eight output channels as four output tiles with 16 software-managed K=8 chunks each. It checks exact INT32 accumulation and a CV32E40X software boundary for bias, Q16 requantization, and INT8 GELU lookup. The remaining output channels and end-to-end inference remain outside the claim; GELU is not claimed as VPU hardware. | `make tinyvit-paper-table`, `make tinyvit-fc1-k128-smoke`, `make tinyvit-fixture-check`, `docs/SAP_VPU_TINYVIT_SMOKE_RECORD.md` |
...
Software bias/GELU execution is now checked for the K=128 eight-output slice.
Full output dimensions, FC2 consumption of the post-GELU activation, and a
defensible power-reduction claim remain subsequent paper-evidence steps. Float
references expose both the original four-channel MLP2 gap and the K=128 FC1
...
2. Widen the validated K=128 FC1 slice beyond eight output channels and feed
   the checked software post-GELU activation into FC2.
