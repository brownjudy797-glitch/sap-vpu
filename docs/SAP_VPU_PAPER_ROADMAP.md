...
| ASIC evidence | A `COMPILE_ULTRA=0` TSMC28 `tt0p9v85c`, 10 ns checkpoint passes the gate-netlist preflight, VCS-MX gate smoke, and DUT-only SAIF annotation with no `PWR-452`. The older RTL-policy matrix still has partial annotation and remains preliminary until regenerated from this clean gate checkpoint. | `make dc-vpu-gate-synth`, `make dc-vpu-gate-saif-power`, `docs/SAP_VPU_ASIC_FLOW.md` |
