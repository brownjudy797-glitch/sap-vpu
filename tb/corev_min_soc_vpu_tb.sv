...
module corev_min_soc_vpu_tb;
  ...
  corev_min_soc #(
    .ROM_INIT_FILE("work/vpu_smoke/sap_vpu_vpu.hex")
  ) dut (
    .clk_i(clk),
    .rst_ni(rst_n),
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    ...
    if (rst_n && exit_valid) begin
      if (exit_code !== 32'd1) begin
        $fatal(1, "VPU smoke exit code expected 1 got %0d", exit_code);
      end
      $display("VPU smoke exit code: %0d", exit_code);
      $finish;
    end
  end
  ...
  initial begin
    ...
    $fatal(1, "Timed out waiting for VPU smoke exit");
  end
endmodule
